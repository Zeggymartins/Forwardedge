  ...
  function openModal(mode, data) {
    ...
    if (!form.dataset.finalizeHooked) {
      form.addEventListener('submit', (e) => {
        ...
        // Step 1: Remove empty items
        pruneEmptyRepeaterItems(mount);

        // Step 2: Remove empty list inputs
        pruneEmptyListInputs(mount);

        // Step 3: Final renumber of everything
        finalizeRepeaters(mount);

        // Step 4: Clean up any leftover placeholders
        stripLeftoverPlaceholders(mount);

        // Step 5: Debug (optional - remove in production)
        debugFormData(mount);
...
<div class="row g-3">
  ...
  <div class="col-md-5"><label class="form-label small-label">Link Text</label><input class="form-control" name="data[link_text]"></div>
  <div class="col-md-7"><label class="form-label small-label">Link</label><input class="form-control" type="url" name="data[link]"></div>
  <div class="col-12"><div class="section-divider"><span class="section-divider-label">Plans</span></div></div>
  <div class="col-12 d-flex justify-content-between">
  ...
  <div class="col-12">
    <div class="repeater" data-repeater data-name="plans">
      <template data-repeater-template>
        <div class="repeater-item border rounded-12 p-3 mb-3" data-repeater-item draggable="true">
          <div class="d-flex gap-3 align-items-start">
            <span class="drag-handle">☰</span>
            <div class="row g-2 flex-grow-1">
              ...
              <div class="col-12">
                <div class="d-flex justify-content-between mb-2 mt-1">
                  ...
                  <button class="btn btn-outline-primary btn-sm" type="button" data-repeater-add="plans___INDEX___features">Add</button>
                </div>
                <div class="repeater" data-repeater data-name="plans___INDEX___features">
                  <template data-repeater-template>
                    <div class="repeater-item d-flex align-items-center gap-2 mb-2" data-repeater-item draggable="true">
                      <span class="drag-handle">☰</span>
                      <input class="form-control" name="data[plans][__INDEX__][features][__SUBINDEX__]">
                      <button class="btn btn-outline-danger btn-sm" type="button" data-repeater-remove>×</button>
                    </div>
                  </template>
                </div>
...
<div class="row g-3">
  ...
  function hydrateNestedArray(scope, parentKey, parentIdx, childKey, childArray) {
    // Wait a bit longer for parent to be fully ready
    setTimeout(() => {
      ...
      if (!addBtn) {
        ...
        console.log('Available add buttons:', [...scope.querySelectorAll('[data-repeater-add]')].map(
          b => b.getAttribute('data-repeater-add')));
        return;
      }

      console.log(`✅ Found nested repeater: ${nestedName}`);
  ...
  function addRepeaterItem(rep, tpl) {
    ...
    // 🔥 CRITICAL: If this repeater has a parent repeater, trigger full renumbering
  ...
  function renumberRepeater(rep) {
    ...
    items.forEach((item, i) => {
      ...
      item.querySelectorAll('[name]').forEach((field) => {
        ...
        field.name = field.name.replace(/\[__INDEX__\]/g, `[${i}]`);
      });
      ...
      // Fix nested renumber logic (replace the broken section)
  ...
  // 🆕 NEW FUNCTION: Handle nested repeater renumbering
  function renumberNestedRepeater(nestedRep, parentIndex) {
    const nestedItems = [...nestedRep.querySelectorAll(':scope > [data-repeater-item]')];

    nestedItems.forEach((nestedItem, j) => {
      // Update all field names in nested items
      nestedItem.querySelectorAll('[name]').forEach((field) => {
        // Replace __SUBINDEX__ with actual index
        field.name = field.name.replace(/\[__SUBINDEX__\]/g, `[${j}]`);
        // Also handle any remaining __INDEX__ that should be the parent index
        field.name = field.name.replace(/\[__INDEX__\]/g, `[${parentIndex}]`);
      });
    });
  }
  ...
  function finalizeRepeaters(root = document) {
    // Process all repeaters - parent and nested
    root.querySelectorAll('.repeater[data-name]').forEach((rep) => {
      renumberRepeater(rep);
    });
  }
  ...
  function pruneEmptyListInputs(root = document) {
    root.querySelectorAll('input[name*="[list]"], textarea[name*="[list]"]').forEach((inp) => {
      if ((inp.value || '').trim() === '') {
        const item = inp.closest('[data-repeater-item]');
        if (item) item.remove();
      }
    });
  }
  ...
  function pruneEmptyRepeaterItems(root = document) {
    // Get ALL repeaters (both parent and nested)
    ...
    allRepeaters.forEach((rep) => {
      ...
      items.forEach((item) => {
        // Check if this item has any required fields
        const requiredFields = item.querySelectorAll('[required]');

        if (requiredFields.length === 0) return; // No required fields, keep item

        // Check if ALL required fields are empty
        const allEmpty = [...requiredFields].every(field => {
          if (field.type === 'file') return true; // Skip file inputs in check
          return !field.value || field.value.trim() === '';
        });

        // Only remove if ALL required fields are empty
        if (allEmpty) {
          console.log('🗑️ Removing empty item with required fields');
          item.remove();
        }
      });
    });

    // After pruning, renumber everything
    finalizeRepeaters(root);
  }
  ...
  function debugFormData(root = document) {
    ...
    // Show all data fields being submitted
